...
export default function Admin() {
  ...
  const handleEditPoints = async (userId: string) => {
    const pts = prompt('New points balance:');
          ...
          <>
            {activeTab === 'users' && (
              <div className="bg-surface border border-border rounded overflow-hidden">
                <div className="grid grid-cols-[2fr_1fr_80px_80px_80px_100px] gap-3 px-5 py-2.5 bg-surface-elevated border-b border-border">
                  {['USER', 'EMAIL', 'BALANCE', 'TRUST', 'STATUS', 'ACTIONS'].map(h => <span key={h} className="label-caps">{h}</span>)}
                </div>
                {users.map(u => (
                  <div key={u.id} className="grid grid-cols-[2fr_1fr_80px_80px_80px_100px] gap-3 px-5 py-3 border-b border-border-subtle last:border-0 hover:bg-surface-elevated transition-colors items-center">
                    ...
                    <span className="font-mono text-xs text-foreground">{u.trust_score?.toFixed(0)}</span>
                    <span className={`label-caps ${u.is_banned ? 'text-spend' : 'text-earn'}`}>{u.is_banned ? 'BANNED' : 'ACTIVE'}</span>
                    <div className="flex gap-1.5">
                      <button onClick={() => handleEditPoints(u.user_id)} className="px-2 py-1 bg-primary/10 border border-primary/20 rounded text-xs text-primary hover:bg-primary/20 transition-colors">PTS</button>
            ...
            {activeTab === 'reports' && (
              <div className="bg-surface border border-border rounded overflow-hidden">
                ...
                {reports.length === 0 ? <p className="p-6 text-sm text-foreground-muted text-center">No reports</p> : reports.map(r => (
                  <div key={r.id} className="grid grid-cols-[2fr_1fr_80px_100px] gap-3 px-5 py-3 border-b border-border-subtle last:border-0 hover:bg-surface-elevated transition-colors items-center">
            ...
            {activeTab === 'payouts' && (
              <div className="bg-surface border border-border rounded overflow-hidden">
                <div className="grid grid-cols-[1fr_80px_80px_80px_120px] gap-3 px-5 py-2.5 bg-surface-elevated border-b border-border">
                ...
                {payouts.length === 0 ? <p className="p-6 text-sm text-foreground-muted text-center">No payout requests</p> : payouts.map(p => (
                  <div key={p.id} className="grid grid-cols-[1fr_80px_80px_80px_120px] gap-3 px-5 py-3 border-b border-border-subtle last:border-0 hover:bg-surface-elevated transition-colors items-center">
            ...
            {activeTab === 'economy' && (
              <div className="bg-surface border border-border rounded overflow-hidden">
                <div className="grid grid-cols-[1fr_1fr_1fr_1fr] gap-3 px-5 py-2.5 bg-surface-elevated border-b border-border">
                  {['PLATFORM', 'TASK', 'EARN PTS', 'COST PTS'].map(h => <span key={h} className="label-caps">{h}</span>)}
                </div>
                {economy.map(row => (
                  <div key={row.id} className="grid grid-cols-[1fr_1fr_1fr_1fr] gap-3 px-5 py-3 border-b border-border-subtle last:border-0 hover:bg-surface-elevated transition-colors items-center">
